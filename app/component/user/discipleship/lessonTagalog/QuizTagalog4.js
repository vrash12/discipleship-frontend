...
const QuizTagalog4 = ({ navigation, route }) => {
  ...
  const fetchQuizData = async () => {
    try {
      // Check if the user has already passed Quiz Tagalog 4
      const statusResponse = await axios.get(
        `https://backend-disciple-a692164f13b9.herokuapp.com/api/quiz/status/4`,
        {
      ...
      const quizResponse = await axios.get(
        `https://backend-disciple-a692164f13b9.herokuapp.com/api/quiz/tagalog/4`
      );
      ...
      if (
        ...
        console.log('Fetched Tagalog Quiz 4 Data:', quizResponse.data);
      } else {
        Alert.alert('No Questions', 'The quiz does not have any questions.');
      }
    } catch (error) {
      console.error('Error fetching Tagalog Quiz 4 data:', error);
  ...
  const handleSubmitQuiz = async () => {
    ...
    const quizResult = {
      quizId: quiz.quizId, // Assuming quizId is 4
      userId: userId,
    ...
    try {
      ...
      if (passed) {
        ...
        try {
          await AsyncStorage.setItem('quiz4Passed', 'true');
        } catch (error) {
