...
const QuizTagalog1 = ({ navigation }) => {
  ...
  const fetchQuizData = async () => {
    try {
      ...
      const statusResponse = await axios.get(
        'https://backend-disciple-a692164f13b9.herokuapp.com/api/quiz/status/1',
        {
      ...
      const quizResponse = await axios.get(
        'https://backend-disciple-a692164f13b9.herokuapp.com/api/quiz/tagalog/1'
      );
      if (
        quizResponse.data &&
        ...
        console.log('Fetched Questions:', JSON.stringify(quizResponse.data.questions, null, 2));
        setLoading(false);
      } else {
  ...
  const validateAnswer = (selectedOption) => {
    ...
    const correctOptionID = currentQuestion.answers.find((option) => option.isCorrect)?.answerID;
  ...
  const handleQuizEnd = () => {
    const quizResult = {
      quizId: 1, // Since this is quiz 1 (Tagalog)
