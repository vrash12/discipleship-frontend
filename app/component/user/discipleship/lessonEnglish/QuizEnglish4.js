...
const QuizEnglish4 = ({ navigation }) => {
  ...
  const fetchQuizData = async () => {
    try {
      ...
      const quizResponse = await axios.get(
        'https://backend-disciple-a692164f13b9.herokuapp.com/api/quiz/english/4'
      );
      if (
        quizResponse.data &&
  ...
  const validateAnswer = (selectedOption) => {
    ...
    const correctOptionID = currentQuestion.answers.find(
  ...
  const handleQuizEnd = () => {
    const quizResult = {
      quizId: 4, // Since this is quiz 4 (English)
