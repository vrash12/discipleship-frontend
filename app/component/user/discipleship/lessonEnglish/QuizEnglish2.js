...
const QuizEnglish2 = ({ navigation }) => {
  ...
  const fetchQuizData = async () => {
    try {
      ...
      const quizResponse = await axios.get(
        'https://backend-disciple-a692164f13b9.herokuapp.com/api/quiz/english/2'
      );
      if (
        quizResponse.data &&
  ...
  const handleQuizEnd = () => {
    const quizResult = {
      quizId: 2, // Since this is quiz 2 (English)
    ...
    axios
      ...
      .catch((error) => {
        ...
        Alert.alert(
          'Error',
          'An error occurred while saving your quiz result.'
        );
      });
  };
